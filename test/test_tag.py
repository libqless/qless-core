...
class TestTag(TestQless):
    ...
    def test_get(self):
        ...
        self.lua(
            "put", 0, "worker", "queue", "foo", "klass", {}, 0, "tags", ["foo", "both"]
        )
        self.lua(
            "put", 0, "worker", "queue", "bar", "klass", {}, 0, "tags", ["bar", "both"]
        )
        self.assertEqual(self.lua("tag", 0, "get", "foo", 0, 10)["jobs"], ["foo"])
        self.assertEqual(self.lua("tag", 0, "get", "bar", 0, 10)["jobs"], ["bar"])
        self.assertEqual(
    ...
    def test_get_add(self):
        ...
        self.lua("put", 0, "worker", "queue", "jid", "klass", {}, 0)
        self.assertEqual(self.lua("tag", 0, "get", "foo", 0, 10)["jobs"], {})
        self.lua("tag", 0, "add", "jid", "foo")
        self.assertEqual(self.lua("tag", 0, "get", "foo", 0, 10)["jobs"], ["jid"])
    ...
    def test_expired_jobs(self):
        ...
        self.assertEqual(self.lua("tag", 99, "get", "foo", 0, 10)["jobs"], ["jid"])
        ...
        self.assertEqual(self.lua("tag", 101, "get", "foo", 0, 10)["jobs"], {})
    ...
    def test_expired_count_jobs(self):
        ...
        self.assertEqual(self.lua("tag", 1, "get", "foo", 0, 10)["jobs"], {})

    def test_top(self):
        """Ensure that we can find the most common tags"""
        for tag in range(10):
            self.lua(
                "put", 0, "worker", "queue", tag, "klass", {}, 0, "tags", range(tag, 10)
            )
        self.assertEqual(
            self.lua("tag", 0, "top", 0, 20), map(str, reversed(range(1, 10)))
        )
    ...
    def test_recurring(self):
        ...
        self.lua("pop", 0, "queue", "worker", 10)
        self.assertEqual(self.lua("tag", 0, "get", "foo", 0, 10)["jobs"], ["jid-1"])
    ...
    def test_pagination_get(self):
        ...
        for jid in jids:
            self.lua(
                "put", jid, "worker", "queue", jid, "klass", {}, 0, "tags", ["foo"]
            )
        # Get two pages and ensure they're what we expect
        self.assertEqual(self.lua("tag", 100, "get", "foo", 0, 50)["jobs"], jids[:50])
        self.assertEqual(self.lua("tag", 100, "get", "foo", 50, 50)["jobs"], jids[50:])
    ...
    def test_pagination_top(self):
        ...
        self.assertEqual(self.lua("tag", 100, "top", 0, 5), jids[:5])
        self.assertEqual(self.lua("tag", 100, "top", 5, 5), jids[5:])
