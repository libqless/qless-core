        ...
        self.assertEqual(
            self.lua("get", 3, "jid"),
            {
                ...
                "history": [
                    ...
                    {"group": "group", "what": "failed", "when": 2, "worker": "worker"},
                ],
                "jid": "jid",
                "klass": "klass",
                "priority": 0,
...
class TestFailed(TestQless):
    ...
    def test_malformed(self):
        ...
        self.assertMalformed(
            self.lua, [("failed", 0, "foo", "foo"), ("failed", 0, "foo", 0, "foo")]
        )
    ...
    def test_basic(self):
        ...
        self.assertEqual(self.lua("failed", 0), {"group": 1})
        self.assertEqual(self.lua("failed", 0, "group"), {"total": 1, "jobs": ["jid"]})
    ...
    def test_retries(self):
        ...
        self.assertEqual(self.lua("failed", 0), {"failed-retries-queue": 1})
        self.assertEqual(
            self.lua("failed", 0, "failed-retries-queue"), {"total": 1, "jobs": ["jid"]}
        )
    ...
    def test_failed_pagination(self):
        ...
        self.assertEqual(self.lua("failed", 0, "group", 0, 50)["jobs"], jids[:50])
        self.assertEqual(self.lua("failed", 0, "group", 50, 50)["jobs"], jids[50:])
