...
class TestJobs(TestQless):
    ...
    def test_recurring(self):
        ...
        jids = map(str, range(0, 10))
        for jid in jids:
            self.lua("recur", jid, "queue", jid, "klass", {}, "interval", 60, 0)
    ...
    def test_recurring_offset(self):
        ...
        jids = map(str, range(0, 10))
        for jid in jids:
            self.lua("recur", jid, "queue", jid, "klass", {}, "interval", 60, 10)
            recurring = self.lua("jobs", 0, "recurring", "queue")
            self.assertEqual(len(recurring), int(jid) + 1)
            self.assertEqual(recurring[-1], jid)
    ...
    def test_pagination_complete(self):
        ...
        self.assertEqual(self.lua("jobs", 0, "complete", 0, 50), jids[:50])
        self.assertEqual(self.lua("jobs", 0, "complete", 50, 50), jids[50:])
    ...
    def test_pagination_running(self):
        ...
        for jid in jids:
            ...
            self.lua("pop", jid, "queue", "worker", 10)
        # Get two pages and ensure they're what we expect
        self.assertEqual(self.lua("jobs", 100, "running", "queue", 0, 50), jids[:50])
        self.assertEqual(self.lua("jobs", 100, "running", "queue", 50, 50), jids[50:])
...
class TestPut(TestQless):
    ...
    def test_malformed(self):
        ...
        self.assertMalformed(
            ...
            [
                ...
                (12345, "foo", "bar", "whiz", "{}"),  # No delay provided
                (12345, "foo", "bar", "whiz", "{]"),  # Malformed data provided
                (
                    12345,
                    "foo",
                    "bar",
                ...
                (
                    ...
                    "retries",
                    "foo",
                ),  # Retries arg not a number
                (12345, "foo", "bar", "whiz", "{}", 1, "tags"),  # Tags arg missing
                (
                    12345,
                    "foo",
                    "bar",
    ...
    def test_put_tags(self):
        ...
        self.lua(
            "put", 12345, "worker", "queue", "jid", "klass", {}, 0, "tags", ["foo"]
        )
...
class TestPeek(TestQless):
    ...
    def test_basic(self):
        ...
        self.assertEqual(
            [o["jid"] for o in self.lua("peek", 3, "foo", 10)], ["jid", "jid2"]
        )
    ...
    def test_priority_update(self):
        ...
        self.assertEqual(
            ["b", "a"], [j["jid"] for j in self.lua("peek", 0, "queue", 100)]
        )
        self.lua("priority", 0, "a", 2)
        self.assertEqual(
            ["a", "b"], [j["jid"] for j in self.lua("peek", 0, "queue", 100)]
        )
...
class TestPop(TestQless):
    ...
    def test_reduce_max_concurrency(self):
        ...
        self.assertEqual(len(self.lua("pop", 100, "queue", "worker", 10)), 1)
