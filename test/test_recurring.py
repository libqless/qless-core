...
class TestRecurring(TestQless):
    ...
    def test_malformed(self):
        ...
        self.assertMalformed(
            ...
            [
                ...
                ("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0, "tags"),
                (
                    "recur",
                    0,
                    "queue",
                ...
                ("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0, "retries"),
                (
                    "recur",
                    0,
                    "queue",
                ...
                ("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0, "backlog"),
                (
                    "recur",
                    0,
                    "queue",
    ...
    def test_priority(self):
        ...
        self.lua(
            "recur", 0, "queue", "high", "klass", {}, "interval", 60, 0, "priority", 10
        )
    ...
    def test_retries(self):
        ...
        self.lua(
            "recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0, "retries", 2
        )
        job = self.lua("pop", 0, "queue", "worker", 10)[0]
    ...
    def test_backlog(self):
        ...
        self.lua(
            "recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0, "backlog", 1
        )
    ...
    def test_update_priority(self):
        ...
        self.lua("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0)
        self.assertEqual(self.lua("pop", 0, "queue", "worker", 10)[0]["priority"], 0)
        self.lua("recur.update", 0, "jid", "priority", 10)
        self.assertEqual(self.lua("pop", 60, "queue", "worker", 10)[0]["priority"], 10)
    ...
    def test_update_retries(self):
        ...
        self.lua(
            "recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0, "retries", 5
        )
        self.assertEqual(self.lua("pop", 0, "queue", "worker", 10)[0]["retries"], 5)
        self.lua("recur.update", 0, "jid", "retries", 2)
        self.assertEqual(self.lua("pop", 60, "queue", "worker", 10)[0]["retries"], 2)

    def test_update_data(self):
        """We need to be able to update the data"""
        self.lua("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0)
        self.assertEqual(self.lua("pop", 0, "queue", "worker", 10)[0]["data"], "{}")
        self.lua("recur.update", 0, "jid", "data", {"foo": "bar"})
        self.assertEqual(
            self.lua("pop", 60, "queue", "worker", 10)[0]["data"], '{"foo": "bar"}'
        )

    def test_update_klass(self):
        """We need to be able to update klass"""
        self.lua("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0)
        self.assertEqual(self.lua("pop", 0, "queue", "worker", 10)[0]["klass"], "klass")
    ...
    def test_multiple(self):
        ...
        self.lua("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0)
        self.assertEqual(len(self.lua("pop", 599, "queue", "worker", 10)), 10)

    def test_tag(self):
        """We should be able to add tags to jobs"""
        self.lua("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0)
        self.assertEqual(self.lua("pop", 0, "queue", "worker", 10)[0]["tags"], {})
        self.lua("recur.tag", 0, "jid", "foo")
        self.assertEqual(self.lua("pop", 60, "queue", "worker", 10)[0]["tags"], ["foo"])

    def test_untag(self):
        """We should be able to remove tags from a job"""
        self.lua(
            "recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0, "tags", ["foo"]
        )
        self.assertEqual(self.lua("pop", 0, "queue", "worker", 10)[0]["tags"], ["foo"])
        self.lua("recur.untag", 0, "jid", "foo")
        self.assertEqual(self.lua("pop", 60, "queue", "worker", 10)[0]["tags"], {})

    def test_rerecur(self):
        """Don't reset the jid counter when re-recurring a job"""
        self.lua("recur", 0, "queue", "jid", "klass", {}, "interval", 60, 0)
        self.assertEqual(self.lua("pop", 0, "queue", "worker", 10)[0]["jid"], "jid-1")
        # Re-recur it
        self.lua("recur", 60, "queue", "jid", "klass", {}, "interval", 60, 0)
        self.assertEqual(self.lua("pop", 60, "queue", "worker", 10)[0]["jid"], "jid-2")
    ...
    def test_rerecur_attributes(self):
        ...
        self.assertEqual(
            self.lua("pop", 0, "queue", "worker", 10)[0],
            {
                "data": "{}",
                "dependencies": {},
                "dependents": {},
        ...
        self.assertEqual(
            self.lua("pop", 60, "queue", "worker", 10)[0],
            {
                "data": '{"foo": "bar"}',
                "dependencies": {},
                "dependents": {},
    ...
    def test_rerecur_move(self):
        ...
        self.assertEqual(self.lua("pop", 60, "other", "worker", 10)[0]["jid"], "jid-2")
    ...
    def test_history(self):
        ...
        self.assertEqual(times, [0, 60, 120, 180, 240, 300, 360, 420, 480, 540])
